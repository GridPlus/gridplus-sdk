...
import { TRANSACTION_TYPE, type TransactionRequest } from "./types";
import {
	buildSignerPathBuf,
	ensureHexBuffer,
	fixLen,
...
const validateEthereumMsgResponse = (res, req) => {
	...
	if (input.protocol === "signPersonal") {
		...
		const chainId = Number.parseInt(input.payload.domain.chainId, 16);
		// Get recovery param with a `v` value of [27,28] by setting `useEIP155=false`
...
const hashTransaction = (serializedTx) =>
	keccak256(Buffer.from(serializedTx, "hex"));
...
// ======
const ethConvertLegacyToGenericReq = (req) => {
	let common;
