...
describe('Connect and Pair', () => {
  ...
  it('Should get the initial set of records', async () => {
    try {
      const resp = await helpers.execute(client, 'getKvRecords', { n: 2, start: 0 })
  ...
  it('Should add some key value records', async () => {
    ...
    try {
      await helpers.execute(client, 'addKvRecords', { records, caseSensitive: false, type: 0 });
    } catch (err) {
      expect(err).to.equal(null, err);
    }
  })
  ...
  it('Should add the same record with case sensitivity', async () => {
    ...
    try {
      await helpers.execute(client, 'addKvRecords', { records, caseSensitive: true, type: 0 });
    } catch (err) {
      expect(err).to.equal(null, err);
    }
  })
