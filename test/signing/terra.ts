...
describe('[Terra]', () => {
  ...
  function getTerraAccount(path) {
    const key = new TerraRawKey(test.helpers.deriveSECP256K1Key(path, test.seed).priv);
  ...
  it('Should decode MsgSend', async () => {
    ...
    test.expect(latticeSig).to.equal(jsSig.toString('hex'), 'Sigs did not match')
  })
  ...
  it('Should decode MsgMultiSend', async () => {
    ...
    test.expect(latticeSigA).to.equal(jsSigA.toString('hex'), 'Sigs did not match')
    // Signer B
    ...
    test.expect(latticeSigB).to.equal(jsSigB.toString('hex'), 'Sigs did not match')
  })
