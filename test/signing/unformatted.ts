...
describe('[Unformatted]', () => {
  ...
  it('Should test pre-hashed messages', async () => {
    ...
    req.data.payload = `0x${test.helpers.prandomBuf(test.prng, maxSz, true).toString('hex')}`;
    await test.runGeneric(req, test);
    if (!test.continue) {
      return;
    }
    // Prehash (keccak256)
    req.data.payload = `0x${test.helpers.prandomBuf(test.prng, maxSz + 1, true).toString('hex')}`;
    await test.runGeneric(req, test);
    if (!test.continue) {
      return;
    }
  ...
  it('Should validate SECP256K1/KECCAK signature against ETH_MSG request (legacy)', async () => {
    ...
    test.expect(genSig).to.equal(legSig, 'Legacy and generic requests produced different sigs.')
  });
