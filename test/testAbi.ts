...
function genRandParam(_type = null) {
  ...
  if (d.isArray) {
    d.name += '[';
    if (d.arraySz > 0) d.name += d.arraySz;
    d.name += ']';
  }
  return d;
}
...
function getTypeNames(params) {
  ...
  params.forEach((param) => {
    ...
    if (param.isArray) {
      typeName += '[';
      if (param.arraySz > 0) typeName += param.arraySz;
      typeName += ']';
    }
...
function genRandVal(type) {
  if (isNumType(type)) return randNumVal(type) || 0;
  else if (type === 'address') return randAddress();
...
function createDef() {
  ...
  if (data.length > maxDataSz) return createDef();
  return def;
}
...
function createBoundaryDefs() {
  ...
  def = {
    ...
    _vals: ['0x00', '0x00', '0x00', '0x00', '0x00', '0x00'],
  };
  def._typeNames = getTypeNames(def.params);
  def.sig = buildFuncSelector(def);
  boundaryAbiDefs.push(def);
  ...
  def = {
    ...
    _vals: [
      ...
      [
        '0x26c6d7ee87cf88537e423a443b3613cbb03b5072',
        '0xaea00d99767f52070b7757dec267fc3c362713d4',
      ],
...
describe('Cleanup', () => {
  ...
  it('Should remove all ABI records added by these tests', async () => {
    const sigs = [];
