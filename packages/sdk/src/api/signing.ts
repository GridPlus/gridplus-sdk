...
import { Hash } from 'ox'
import { type Address, type Authorization, type Hex, type TransactionSerializable, type TransactionSerializableEIP7702, serializeTransaction } from 'viem'
import { Constants } from '..'
import { BTC_LEGACY_DERIVATION, BTC_SEGWIT_DERIVATION, BTC_WRAPPED_SEGWIT_DERIVATION, CURRENCIES, DEFAULT_ETH_DERIVATION, SOLANA_DERIVATION } from '../constants'
import { fetchDecoder } from '../functions/fetchDecoder'
import type { BitcoinSignPayload, EIP712MessagePayload, SignData, SignRequestParams, SigningPayload, TransactionRequest } from '../types'
...
export const sign = async (transaction: TransactionSerializable | RawTransaction, overrides?: Omit<SignRequestParams, 'data'>): Promise<SignData> => {
	const isRaw = isRawTransaction(transaction)
	const serializedTx = isRaw ? normalizeRawTransaction(transaction) : serializeTransaction(transaction as TransactionSerializable)

	// Determine the encoding type based on transaction type
	let encodingType: typeof Constants.SIGNING.ENCODINGS.EVM | typeof Constants.SIGNING.ENCODINGS.EIP7702_AUTH | typeof Constants.SIGNING.ENCODINGS.EIP7702_AUTH_LIST = Constants.SIGNING.ENCODINGS.EVM
	if (!isRaw && (transaction as TransactionSerializable).type === 'eip7702') {
		...
		encodingType = hasAuthList ? Constants.SIGNING.ENCODINGS.EIP7702_AUTH_LIST : Constants.SIGNING.ENCODINGS.EIP7702_AUTH
	}

	// Only fetch decoder if we have the required fields
	let decoder: Buffer | undefined
	if (!isRaw && 'data' in (transaction as TransactionSerializable) && 'to' in (transaction as TransactionSerializable) && 'chainId' in (transaction as TransactionSerializable)) {
 ...
 */
export function signMessage(payload: string | Uint8Array | Buffer | Buffer[] | EIP712MessagePayload<Record<string, unknown>>, overrides?: Omit<SignRequestParams, 'data'>): Promise<SignData> {
...
function isRawTransaction(value: TransactionSerializable | RawTransaction): value is RawTransaction {
 ...
 */
export const signAuthorization = async (authorization: AuthorizationRequest, overrides?: Omit<SignRequestParams, 'data'>): Promise<Authorization> => {
	...
	const address = 'address' in authorization ? authorization.address : authorization.contractAddress

	const message = Buffer.concat([MAGIC, Buffer.from(RLP.encode([authorization.chainId, address, authorization.nonce]))])
	...
	const response = await queue((client) => client.sign({ data: payload, ...overrides }))
	...
	if (response.sig && response.pubkey) {
		...
		const rValue = Buffer.isBuffer(response.sig.r) ? `0x${response.sig.r.toString('hex')}` : response.sig.r
		const sValue = Buffer.isBuffer(response.sig.s) ? `0x${response.sig.s.toString('hex')}` : response.sig.s
 ...
 */
export const signAuthorizationList = async (tx: TransactionSerializableEIP7702): Promise<SignData> => {
...
export const signBtcLegacyTx = async (payload: BitcoinSignPayload): Promise<SignData> => {
	const tx = {
		data: {
...
export const signBtcSegwitTx = async (payload: BitcoinSignPayload): Promise<SignData> => {
	const tx = {
		data: {
...
export const signBtcWrappedSegwitTx = async (payload: BitcoinSignPayload): Promise<SignData> => {
	const tx = {
		data: {
...
export const signSolanaTx = async (payload: Buffer, overrides?: SignRequestParams): Promise<SignData> => {
	const tx = {
		data: {
